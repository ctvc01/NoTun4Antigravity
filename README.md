...
* ⚡ **全链路 AI 探活三矩阵 (v1.3)**：并发深度探测 Gemini API 模型通道、CloudCode 调度通道与 OAuth2 令牌刷新通道，精准排查“运行两步即断开”的断连元凶与 TLS 握手损坏。
* 🛡️ **单主开关全能聚合 (v1.3)**：遵循极简设计（Ponytail 原则），将动态端口感知、进程环境变量注入、本地 IDE `settings.json` 防死锁托管、SSH 心跳防断连与 QUIC 阻断完全聚合于单个主开关，后台静默自愈，零心智负担。
...
1. **状态检查与一键守护**：确认「全局代理与环境守护」主开关开启（默认已开启一键全链路自动托管）。若你的代理客户端监听在其他端口，点击「修改」即可自定义或重测。
2. **节点真实网络探测与筛选**：点击「节点真机测速与筛选」，可一览当前节点的 Google 连通度与 Gemini AI 探活三矩阵状态（模型流、CloudCode、OAuth2）。
3. **配置直连白名单**：点击「直连白名单规则」，输入需要跳过代理的公司内网域名（每行一条）。
4. **一键生效**：点击右下角 **「应用配置&重启」**，工具将安全重启并以全新网络参数拉起 Antigravity。
