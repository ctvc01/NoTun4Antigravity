...
* 🔧 **本地 IDE 设置智能防死锁同步 (v1.3)**：联动同步本地 `settings.json`（`http.proxySupport: "override"`, `http.proxyStrictSSL: false`），消除旧代理残留与证书链死锁。
* 🚀 **远程开发与 SSH 保活助手 (v1.3)**：一键优化本地 `~/.ssh/config` 心跳保活（防止 VPN 闲置断线），并提供免终端一键远程 Linux 服务器代理环境穿透修复。
