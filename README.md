...
* ⚡️ **测速节点列表去繁化简与智能净滤 (v1.8)**：彻底移除「仅专线」与「过滤失效」复选开关，默认且强制实施工业级全自动净滤。深度结合 3400+ 条审计日志，自动剔除断连、超时、>2s 虚高假通节点，以及特殊用途、游戏、下载、直连家宽等不具备 Google Gemini AI 原生公网能力的劣质节点（彻底根治台湾07等节点 TCP 通但 AI 受限的漏网隐患）；活动节点 AI 受限时醒目标红预警。
* 🧹 **孤儿代理智能自愈与全系统防断网 (v1.7)**：代理客户端关闭或退出时，秒级感知并自动清理 macOS 系统残留的 HTTP/HTTPS/SOCKS 代理设置，全系统（钉钉、微信、浏览器等）瞬间恢复直连上网，从根本上终结死端口断网与无限加白名单的历史恶疾。
...
* 🛡️ **内存级系统网络代理守护与自愈 (v1.5)**：0.05ms 微秒级无感知内存轮询，自动并集合并系统代理白名单，无缝解决 FlClash/Clash 误冲内网白名单引发的 502/断网难题，全生态加白携程内网。
