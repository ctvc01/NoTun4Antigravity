...
* ⚡️ **真实检测驱动与动态测速过滤 (v1.8)**：移除「仅专线」与「过滤失效」冗余开关，不依赖名称主观一刀切，完全以真实测速检测结果为准动态过滤。自动剔除断连、超时以及 >2s 的高丢包假通节点；每次手动测速时全量重新审查所有订阅节点，之前断开被过滤的节点在最新测速中恢复可连通后立即自动解除过滤重新展示；活动节点若 AI 服务受限坚决标红预警，彻底消除虚假通畅假象。
